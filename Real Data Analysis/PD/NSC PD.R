...
NSC_Gender <- foreach(j = 1:R,.combine = rbind,
                        .packages = c("pamr","tensr","ramify","stats",'rTensor','expm',"TULIP",'glmnet','splitTools')) %dopar% {
                          ...
                          Re=pamr.predict(mod1,t(Xtr),threshold = CV1$threshold[a3[length(a3)]])
                          ret <- list(er = sum(Re != Ytr)/nsam*100,lambda = CV1$threshold[a3[length(a3)]])
                          ret   
                        }

NSC_Gender <- as.data.frame(NSC_Gender)
error <- as.numeric(NSC_Gender$er)
