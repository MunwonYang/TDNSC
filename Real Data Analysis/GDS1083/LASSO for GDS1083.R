...
GDS1083_LASSO <- foreach(j = 1:R,.combine = rbind,
                  .packages = c("pamr","tensr","ramify","stats",'rTensor','expm',"TULIP",'glmnet','splitTools')) %dopar% {
                    ...
                    ret <- list(er = sum((res1-1) != Ytr)/nsam*100,lambda = CV1$lambda.min)
                    ret   
                  }
GDS1083_LASSO <- as.data.frame(GDS1083_LASSO)
error <- as.numeric(GDS1083_LASSO$er)
