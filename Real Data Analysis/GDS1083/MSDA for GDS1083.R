# This code obtains the "MSDA" classification result on GDS1083(Multi-categorical classification problem)
rm(list = ls())
...
GDS1083_MSDA <- foreach(j = 1:R,.combine = rbind,
                       .packages = c("pamr","tensr","ramify","stats",'rTensor','expm',"TULIP",'glmnet','splitTools')) %dopar% {
                         ...
                         mod1 <- msda(x = Xt, y = Yt,maxit = 1e4)
                         # Cross-validation for the model
                         modcv <- cv.msda(x = Xt, y = Yt, lambda = mod1$lambda, maxit = 1e4)
                         # Fit the model with optimal lambda value
                         Mod1 <- msda(x = Xt, y = Yt, lambda = modcv$lambda.min)
                         # Estimate class label on test set by using trained model
                         Re <- predict.msda(Mod1, Xtr)
                         ret <- list(er = sum(Re != Ytr)/nsam*100,lambda = modcv$lambda.min)
                         ret   
                       }
GDS1083_MSDA <- as.data.frame(GDS1083_MSDA)
error <- as.numeric(GDS1083_MSDA$er)
