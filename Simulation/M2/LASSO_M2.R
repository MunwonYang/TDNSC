# This code obtains the "LASSO" result on Model M2
rm(list = ls())
...
LASSO_M2 <- foreach(j = 1:R,.combine = rbind,
                    .packages = c("pamr","tensr","ramify","stats",'rTensor','expm',"TULIP",'glmnet')) %dopar% {
                      ...
                      cv <- cv.glmnet(x = X,y = Y, family = 'multinomial', nfolds = 5)
                      
                      # Fit the model with optimal lambda value 
                      fit <- glmnet(x = X,y = Y,lambda = cv$lambda.min,family = 'multinomial',alpha = 1)
                      ...
                      Yhat <- argmax(prediction,row = TRUE)
                      ret <- list(er = sum(Yhat != Y)/n*100,lambda = cv$lambda.min)
                      ret                          
                    }
LASSO_M2 <- as.data.frame(LASSO_M2)
error <- as.numeric(LASSO_M2$er)
print("M2 LASSO")
