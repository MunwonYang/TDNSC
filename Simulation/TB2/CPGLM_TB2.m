...
rng(1996); % Reset random seed
p1 = 30; p2 = 36;p3 =30;
p = p1*p2*p3;
n1 = 75;n2 = 75;
n = n1+n2;
prob1 = n1/n; prob2 = n2/n;
D11 = [1,2,11,12];
D12 = [1,11];
D13 = 1;
D21 = D11;
...
v1 = zeros([p1,p2,p3]);
v2 = zeros([p1,p2,p3]);
v1(D11,D12,[D13,D23]) = 0.35;
v2(D11,D12,[D13,D23]) = -0.35;
...
% Setting for generating tensor normal distribution(3-way)
dsigma = {};
for i = 1:numel(dimen)
...
mu = {};
for i = 1:K
    mu{i} = ttm(tensor(V{i}),dsigma,[1,2,3]);
end
...
parfor (run = 1:Run,cluster)
    ...
    for i = 1:n
        ...
        X(:,:,:,i) = ttm(Z,dsigma,[1,2,3]) + mu{Y(i)};
    end

    for i = 1:n % The element of X shows as 'real+complex', delete the complex part(i)
        X(:,:,:,i) = real(X(:,:,:,i));
    end

    Rank = [1,2,3]; % CP rank(1,2,3)
    lambda=[0.001,0.005,0.01,0.1,0.3,0.5,1,5,10,15, 20,30, 50]; % Candidate Lambda value
    ...
    for i = 1:length(Rank)
        for j = 1:length(lambda)
            ...
            for k =1:nfold
                ...
                xsmall = array_resize(Train, [10,12,10,size(Train,4)]);
    ...
    for i = 1:n
        Z = tensor(randn(dimen));
        X1(:,:,:,i) =  ttm(Z,dsigma,[1,2,3]) + mu{Y(i)};
    end 

    for i = 1:n % The element of X1 shows as 'real+complex', delete the complex part(i)
        X1(:,:,:,i) = real(X1(:,:,:,i));
    end

    % Rough estimate from reduced sized data
    Xsmall = array_resize(X, [10,12,10,size(X,4)]);
    Z = ones(length(Y),1);
    ...
    err(run) = sum(Y ~= pred) / length(Y) * 100; % Error rate for each simulation
