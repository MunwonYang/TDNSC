# This code obtains the "MSDA" classification result on Model TB1
rm(list = ls())
...
MSDA_TB1 <- foreach(j = 1:R,.combine = rbind,
                    .packages = c("pamr","tensr","ramify","stats",'rTensor','expm',"TULIP","glmnet")) %dopar% {
                      ...
                      cv <- cv.msda(x = X,y = Y, nfolds = 5)
                      
                      # Fit the model with optimal lambda value 
                      fit <- msda(x = X,y = Y,lambda = cv$lambda.min)
...
MSDA_TB1 <- as.data.frame(MSDA_TB1)
error <- as.numeric(MSDA_TB1$er)
print("TB1 MSDA")
